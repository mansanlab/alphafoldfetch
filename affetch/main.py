...
async def alphafold_api_coroutine(
    session: aiohttp.ClientSession, sem: asyncio.Semaphore, url: str, results: ResultMap
) -> None:
...
async def alphafold_api_call(sem: asyncio.Semaphore, urls: list[str], results: ResultMap) -> None:
    async with aiohttp.ClientSession() as session:
        tasks = [alphafold_api_coroutine(session, sem, url, results) for url in urls]
        await asyncio.gather(*tasks)


async def alphafold_api(urls: list[str], n_sync: int) -> dict[str, str]:
